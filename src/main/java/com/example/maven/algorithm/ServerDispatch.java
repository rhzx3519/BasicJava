...
public class ServerDispatch {
    ...
    private int binaryInsert(List<Integer> st, int target) {
        int l = 0, r = st.size() - 1;
        while (l <= r) {
            int mid = l + (r - l)/2;
            int val = st.get(mid);
            if (val == target) {
                while (mid < st.size() - 1 && st.get(mid)  == st.get(mid+1)) {
                    mid++;
                }
                return mid + 1;
            } else if (val > target) {
...
//        int[] load = new int[]{1000000000,1,1000000000};
        ServerDispatch dispatch = new ServerDispatch();
        List<Integer> ans = dispatch.busiestServers(k, arrival, load);
        log.info("ans: {}", ans);

        List<Integer> st = Lists.newArrayList(0, 1);
        int idx = dispatch.binaryInsert(st, 0);
        log.info("idx: {}", idx);
    }
}
